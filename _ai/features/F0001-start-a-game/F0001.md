...
page — on **each** colour, at random positions). After the call the
user sees a fully rendered position that matches the returned `Overview` exactly.
...
each including its king). The `Overview` model is produced by the back end and
consumed by the front end; it is never sent by the caller.
...
| `random_overview` | `game::overview` · `random_overview(pieces: u8) -> Overview` | Place exactly `pieces` of each colour at random positions (rules **B-3**–**B-8**). |
...
3. **The server builds the position — data, not UI.** `start_game_handler`
   ...
   *exactly* `pieces` of each colour at *random* positions (rules **B-2**–**B-8**).
...
10. **F-1 — 64 squares.** `renderBoard` builds an 8×8 grid of square elements.
...
11. **F-2 — Square colour.** `squareColor(square)` returns `dark` for `a1`. A
    square is **dark** when `(file_index + rank_index)` is even and **light**
    when odd, using `a`=1…`h`=8 and rank `1`=1…`8`=8.

12. **F-3 — `Overview.board` drives the pieces.** `cells(overview)` reads
...
13. **F-4 — Grid first, then pieces; grid reused on re-render.** `renderBoard`
...
14. **F-5 — Exact match.** The set of occupied squares and the piece on each one
    matches `overview.board` with no extra or missing pieces. An all-`""` row
    renders eight empty squares.

15. **F-6 — Edge labels.** File marks `a`–`h` run along the bottom edge and rank
    marks `1`–`8` run along the left edge, aligned to their files/ranks.

16. **F-7 — Responsive & square.** The board keeps a 1:1 aspect ratio and scales
    with its container/viewport; it never distorts. Labels and squares scale
    together.

17. **F-8 — Single network seam.** Only `infra.startGame` calls `GET
...
| `Overview` generation fails (random) | layout routine         | `500 Internal Server Error` (should not occur if **B-3**–**B-8** hold) |
...
| `?mode=random&pieces=4`     | `board` with `K` e1, `Q` d1, `N` c5, `P` e2 (white), `k` e8, `p` d7, `n` b6, `q` f4 (black); `white/black = "none"` | exactly 4 white + 4 black, one king each, random squares. |
...
the per-colour count stays fixed at `pieces`, within rules **B-3**–**B-8**.
...
- **Move-legality of random layouts** — random is a dev render aid, not a legal
  game; only **B-4**/**B-6** sanity constraints apply.
