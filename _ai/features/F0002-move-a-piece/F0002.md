...
| `onSquareClick` | `events` · `onSquareClick(square: string): void` | The selection state machine. First click sets `square1` (front checks, rules **F-1**/**F-2**); clicking `square1` again deselects (rule **F-3**); a second click sets `square2` and drives the move (rule **F-4**). |
...
1. **Two clicks pick the move.** `events.onSquareClick` holds `square1`. The
   first click on a white piece sets `square1` and highlights it (rules **F-1**,
...
16. **F-1 — First click selects a white source.** With no `square1` held, a click
    on a square carrying a white piece sets `square1` and highlights it. The front
...
- **Unit tests** — [UT-F0002](unit_test_F0002.md): `domain.isWhitePiece`
  ...
  **smart redraw** (rule **F-5**): feeding a `200` `overview` patches only the
  ...
  every other square's DOM element untouched.
